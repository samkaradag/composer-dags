...
dag = DAG('scd_load_parametric',catchup=False, default_args=default_args, schedule_interval=schedule_interval)
...
# Load TABLE3 from staging to curated
# Initial load table3
initial_load_table3 = BigQueryOperator(
    task_id='initial_load_table3',
    use_legacy_sql=False,
    write_disposition='WRITE_TRUNCATE',
    allow_large_results=True,
    sql=f'''
    #standardSQL
    SELECT *, CURRENT_DATETIME() AS start_date, DATETIME "9999-12-31 23:59:59"  AS end_date FROM `{PROJECT_ID}.{STAGING_DATASET}.table3`''', destination_dataset_table=f'{CURATED_DATASET}.table3',
    dag=dag)


# prepare merge statement for table3
def _create_SCD2_merge_sql(source, target, schema, keys, ti, **kwargs):
    ...
    if 'P_DATE_FIELD' in kwargs['dag_run'].conf:
        filter_mode=True
        filter_value = kwargs['dag_run'].conf['P_DATE_FIELD']
        filter_clause = "WHERE S.DATE_FIELD='" + filter_value +"' "
    else:
        filter_mode=False
        filter_clause = ""
...
table_name='table3'
schema = {'KEY1':'INTEGER','DATE_FIELD':'STRING'}
keys = {'KEY1','DATE_FIELD'}
...
curate_table3 = BigQueryOperator(
    task_id=f"scd2_merge_table3_into_curated",
    sql="{{ti.xcom_pull(key='merge_sql', task_ids='prep_merge')}}" ,
    use_legacy_sql=False,
    dag=dag
)

def set_table3_task( **kwargs):
    ...
    if load_mode == 'INITIAL':
        return 'initial_load_table3'
    elif load_mode == 'DELTA':
        return 'prep_merge'

branch_table3 = BranchPythonOperator(
    task_id='branch_table3',
    python_callable=set_table3_task,
...
#table3
branch_table3.set_upstream(start_curation)
initial_load_table3.set_upstream(branch_table3)

prep_merge.set_upstream(branch_table3)
curate_table3.set_upstream(prep_merge)

finish_curation.set_upstream(curate_table3)
finish_curation.set_upstream(initial_load_table3)